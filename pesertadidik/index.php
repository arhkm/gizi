<?php
  ...
  @$nama_form ="pesertadidiksiswa/hal_pesertadidiks.php?page=home";
	...
	if (isset($_POST['login'])) {
			$sql = mysqli_query($koneksi, "SELECT * FROM tb_siswa WHERE NIS ='$_POST[nis]'");
      $jk = mysqli_fetch_array($sql);
      if (empty($jk)) {
        echo "<script>alert('nis tidak terdaftar');</script>";

      }elseif ($jk['JK']=="L") {
        $perintah->logins($koneksi,$table, $nis,$jk, $nama_form, $nama_form);

      }else{
		$perintah->logins($koneksi,$table, $nis,$jk, $nama_form, $nama_form1);
	}
  }
