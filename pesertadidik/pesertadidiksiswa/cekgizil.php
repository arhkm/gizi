<?php 
      ...
      @$tanggal = date("Y-m-d");
      @$nama = @$user[0];
      $redirect ="hal_pesertadidiks.php?page=cekmakanan";
    ...
    if (isset($_POST['kegiatannya'])) {
      if (empty($_POST['tinggi'] ) || empty($_POST['berat'] ) || empty($_POST['usia'] )) {
        ...
        // echo $ceks;
          if($ceks == 0){
            $nilainya = mysql_fetch_array(mysql_query("SELECT nilai as nil FROM kegiatanl WHERE id = '$_POST[kegiatannya]'"));
            // echo $nilainya['nil'];
            $nilai = $nilainya['nil'];
            $usia = $_POST['usia'];
            $tinngi = $_POST['tinggi'];
            ...
            $bbi = $tb - $tingginya;
            //hasilamb  
            $amb = 66 + (13.7 * $bbi) + (5 *$tinngi)- (6.8 * $usia);
            //hasilkebenergi
            $k1 = $amb * 1.55;
            $ke = $amb * 1.55 * 0.1;
            $ke2 = $amb * $nilai;
            ...
            // echo "$hprot1 - $hprot2 gram -- $hlem1-$hlem2 -- $hkar1-$hkar2 --";
